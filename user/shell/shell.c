...
static struct Command commands[] =
{
	...
	{"touch", "touch <filename> \n\t create new empty file", shell_touch},
        ...
        {"help", "help \n\t print this help message\n\t process commands: LS, CAT, ROT13, and pipelines with |", shell_help},
        {"kill", "kill <signal> <pid> \n\t send signal to process", shell_kill},
        {"trap", "trap <signum> <handler> \n\t register signal handler", shell_trap},
        {"spawn", "spawn <elf_id> \n\t spawn a new process (1=ping, 2=pong, 3=ding, 4=fstest, 5=shell, 6=cat, 7=rot13, 8=echo, 9=ls)", shell_spawn}
};
...
int shell_cd(int argc, char** argv)
{
	char path[1024];
	if (argc == 1) {
		/* BUG FIX: strcpy(path, '\0') passed a char-as-pointer → crash.
		 * Correct: set path to an empty string and chdir to root/home. */
		path[0] = '\0';
		sys_chdir(path);
	} else {
		strcpy(path, argv[1]);
...
int shell_rm(int argc, char** argv)
{
        ...
        /* BUG FIX: was `pathIdx > argc + 1` (inverted / off-by-one).
         * Correct: pathIdx must be a valid argv index, i.e. < argc. */
        if(pathIdx >= argc){
...
int is_dir(char * path){
      int fd, isDir;
      /* BUG FIX: original code called sys_is_dir(fd) with uninitialized fd
       * when is_file_exist() returned 0. Guard properly. */
      if(!is_file_exist(path))
            return 0;
...
int shell_cat(int argc, char** argv)
{
	/* BUG FIX: error message said "touch failed" — should say "cat failed" */
	if (argc == 1) {
...
int _shell_cat(char * path){
   ...
   /* BUG FIX: original read only 99 bytes once — now loop until EOF */
...
{
    return strcmp(name, "CAT") == 0 || strcmp(name, "LS") == 0 || strcmp(name, "ROT13") == 0;
}
...
static int parse_stage(char *cmd, stage_t *stage)
{
    int argc = 0;
    while (1) {
...
static int parse_pipeline(char *buf, stage_t *stages, int *nstages)
{
    char *seg;
    int n = 0;
    seg = buf;
...
static void shell_yield_briefly(void)
{
    int i;
    for (i = 0; i < 2000; i++)
        yield();
...
static int launch_process_stage(stage_t *stage, int stdin_fd, int stdout_fd)
{
    int elf_id;
    char cwd[BUFLEN];
    int input_fd;
...
static int run_pipeline(char *buf)
{
    ...
    if (parse_pipeline(buf, stages, &nstages) < 0)
        return 0;
    if (nstages == 0)
        return 0;
...
int shell_kill(int argc, char **argv)
{
    if (argc < 3) {
        ...
        /* BUG FIX: bare return; in int function — must return a value */
        return 0;
    }
    ...
    /* Parse signal number — handle both "-N" and "N" formats */
    if (argv[1][0] == '-')
...
int shell_spawn(int argc, char **argv)
{
    if (argc < 2) {
        ...
        printf("  elf_id: 1=ping, 2=pong, 3=ding, 4=fstest, 5=shell, 6=cat, 7=rot13, 8=echo, 9=ls\n");
        return -1;
    }
