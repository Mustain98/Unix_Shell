...
void sys_read(tf_t *tf)
{
  ...
  // For pipes and console, ip can be NULL
...
/**
 * Create a pipe and allocate two file descriptors.
 * Parameters (passed via user buffer):
 *   a[0]: pointer to int[2] array in user space for storing fd[0] (read) and fd[1] (write)
 *
 * Return Value: 0 on success, -1 on error
 * Error codes: E_BADF if file table is full, E_INVAL_ADDR for invalid address
 */
void sys_pipe(tf_t *tf)
{
  unsigned int user_fds_addr;
  int fds[2];
  struct file *rf, *wf;
  unsigned int pid = get_curid();
